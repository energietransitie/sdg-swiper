...
import { Animated, Dimensions, InteractionManager, PanResponder, Text, View } from 'react-native';
...
const rebuildStackAnimatedValues = props => {
  ...
  for (let position = 0; position < stackSize; position++) {
    stackPositionsAndScales[`stackPosition${position}`] =
      new Animated.Value(stackSeparation * position);
...
class Swiper extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      previousCardX: new Animated.Value(
        props.previousCardDefaultPositionX
      ),
      previousCardY: new Animated.Value(
        props.previousCardDefaultPositionY
      ),
    ...
    this.setState(
      ...
      () => {
        this.animatePreviousCard(
          this.calculateNextPreviousCardPosition(),
          cb
        );
      }
    );
  ...
  setSwipeBackCardXY = (x = -width, y = 0, cb) => {
    this.setState(
      {
        swipeBackXYPositions: [
          ...this.state.swipeBackXYPositions,
          { x, y }
        ]
      },
      cb
  ...
  animateStack = () => {
    ...
    while (stackSize-- > 1 && showSecondCard && !swipedAllCards) {
      if (
        ...
        const newSeparation =
          this.props.stackSeparation * (stackSize - 1);
        const newScale =
          (100 - this.props.stackScale * (stackSize - 1)) * 0.01;
        Animated.parallel([
  ...
  rebuildStackValues = () => {
    ...
    for (let position = 0; position < stackSize; position++) {
      stackPositionsAndScales[`stackPosition${position}`] =
        new Animated.Value(stackSeparation * position);
      stackPositionsAndScales[`stackScale${position}`] =
        new Animated.Value((100 - stackScale * position) * 0.01);
    }
    return stackPositionsAndScales;
  };
  ...
  calculateOverlayLabelStyle = () => {
    const dynamicStyle =
      this.props.overlayLabels[this.state.labelType].style;
  ...
  calculateOverlayLabelWrapperStyle = () => {
    ...
    const width = this.props.cardWidth + 200
  ...
  renderChildren = () => {
    const { childrenOnTop, children, stackSize, showSecondCard } =
      this.props;
